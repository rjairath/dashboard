...
export const Desktop = ({
	...
	const handleLinkClick = (event: any, name: string) => {
		...
		if (name == 'Work') {
			payload.clickEvent = clickEventsEnum.workSection_click;
			postAnalytics(
				baseUrl,
				date,
			...
			payload.clickEvent = clickEventsEnum.analyticsSection_click;
			postAnalytics(
				baseUrl,
				date,
				analyticsTypeEnum.clickEvent,
		...
		<>
			{links.map((navLink, index) => (
				<Link href={navLink.link} key={index}>
					<div className="relative rounded-lg px-3 inline-block py-2 text-sm text-gray-700 dark:text-gray-200">
						<span
...
const Header = () => {
	...
	const handleThemeClick = () => {
		...
		const date = getDate(0);
		const payload = {
			clickEvent: clickEventsEnum.theme_click,
		};
		...
		>
			<div className="flex justify-between items-center max-w-6xl p-8 mx-auto">
				<div
					...
					className="cursor-pointer dark:text-white p-2 rounded-3xl shadow-lg shadow-zinc-800/5 dark:shadow-white/5"
				>
					{theme ? (
						theme === 'light' ? (
							<Moon />
